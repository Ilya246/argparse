...
inline T __default_read(std::string_view in_str) {
    ...
    T val;
    read_ss >> val;
    if (!read_ss.eof()) throw std::runtime_error("didn't EOF while reading value (" + (std::string)in_str + ")");
    return val;
}
...
inline bool parse_value<>(std::string_view in_str) {
    if (in_str.size() == 0) throw std::runtime_error("tried to parse empty string as bool");
    ...
    if (in_str == "true" || in_str == "yes" || firstc == 't' || firstc == 'y' || firstc == '+' || firstc == '1') {
        ...
        throw std::runtime_error("invalid boolean value " + (std::string)in_str);
    }
}
...
template<typename T>
inline T __parse_tuple(std::string_view in_str) {
    if (in_str.empty()) throw std::runtime_error("tuple is empty");
    T read_tup;
...
template<typename T>
inline T __parse_vector(std::string_view in_str) {
    if (in_str.empty()) throw std::runtime_error("vector is empty");
    T read_vec;

    // first separator is the opening {
    if (in_str[0] != collection_open) throw std::runtime_error("vector lacks opening bracket");
